function buildMonthData() {
  ...
  return Array.from(dateSections).map(section => ({
    date: section.getAttribute('data-date'),
    element: section,
    year: parseInt(section.getAttribute('data-date').split('.')[2], 10),
    month: parseInt(section.getAttribute('data-date').split('.')[1], 10) - 1,
    day: parseInt(section.getAttribute('data-date').split('.')[0], 10)
  }));
}

function setupFilter() {
  const allDates = buildMonthData();
  ...
  function filterByMonth(year, month) {
    allDates.forEach(item => {
      item.element.style.display = (item.year === year && item.month === month) ? 'block' : 'none';
    });
  }
...
document.addEventListener('DOMContentLoaded', () => {
  setupFilter();
  document.addEventListener('changelog:updated', setupFilter);
});